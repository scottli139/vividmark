...
export type FormatType = 'bold' | 'italic' | 'strike' | 'code' | 'link' | 'h1' | 'h2' | 'h3' | 'quote' | 'list' | 'codeblock'
...
export function useTextFormat() {
  const formatText = useCallback((text: string, format: FormatType, selection?: { start: number; end: number }) => {
    ...
    if (selection && selection.start !== selection.end) {
      ...
      const newCursorPos = selection.start + config.prefix.length + selected.length + config.suffix.length
  ...
  const toggleBlockPrefix = useCallback((text: string, format: 'h1' | 'h2' | 'h3' | 'quote' | 'list') => {
    const config = FORMAT_CONFIGS[format]
