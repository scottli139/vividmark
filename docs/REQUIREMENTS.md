...
| FR-003.5 | 支持查找与替换（Source/Split；WYSIWYG 未接） | P0 | ✅ |
| FR-003.6 | 支持全文搜索 | P1 | ⬜ |
...
| FR-011.4 | 外部修改监控（干净自动重载 / 脏冲突弹窗抉择 / 外部删除提示、保存重建） | P1 | ✅ |
...
> 2026-08-13 方向调整：原「多标签页」需求改为 **Typora 式多窗口（SDI）**——每打开一个文档对应一个独立完整窗口，窗口管理交给 OS（macOS `Cmd+\`` 切换、Window 菜单）。下表按此重述：

| ID | 需求描述 | 优先级 | 状态 |
|:---:|---------|:------:|:----:|
| FR-013.1 | 同时打开多个文件（各自独立窗口） | P1 | ✅ |
| FR-013.2 | 已打开文件再次打开时聚焦已有窗口 | P2 | ✅ |
| FR-013.3 | 关闭窗口前提示未保存修改 | P1 | ✅ |
| FR-013.4 | 窗口快捷键（新建窗口 Cmd+N，OS 级窗口切换） | P1 | ✅ |
...
| FR-040.5 | 导出时包含图片资源（PDF 经 asset 协议加载本地图、站点导出复制资产目录） | P1 | ✅ |
| FR-040.6 | 打印预览与打印 | P1 | ⬜ |
...
| Ctrl+F | 查找（Source/Split，查找面板内置替换行；桌面端另有原生菜单 Find） | ✅ |
| Ctrl+H | 替换（查找面板内置） | ✅ |
| Ctrl+B | 粗体 | ✅ |
| Ctrl+I | 斜体 | ✅ |
| Ctrl+K | 插入链接 | ✅ |
| Ctrl+Shift+L | 插入代码块 | ✅ |
| Ctrl+/ | 视图模式切换（WYSIWYG/Source；已让位，注释功能不接） | ✅ |
...
| 1.1.0 | 2026-08-21 | - | 状态全量同步至 v0.8.0 实现现状（FR-001.5/002.3/003.5/004.1-3/010.6/011.4/031.4/040.5/040.8、快捷键表） |
