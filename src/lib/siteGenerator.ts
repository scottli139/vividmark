 ...
 *
 * 导航按目录结构自动推导：数字前缀（01-intro.md）决定排序并从显示名剥离；
 * README.md / index.md 成为所在目录的 index.html。
 */

/** 导航条目 */
export interface SiteNavEntry {
  type: 'page' | 'dir'
  /** 显示名（已剥离数字前缀；页面优先取首个 H1，由 titles 注入） */
  title: string
  /** 页面输出相对路径（'/' 分隔，如 guide/intro.html）；dir 无 */
  htmlPath?: string
  /** 页面源文件绝对路径；dir 无 */
  sourcePath?: string
...
/** 提取页面标题：首个 `# H1`，由调用方注入 buildNavModel 的 titles */
...
/** 文件名回退标题：剥离数字前缀与扩展名 */
function fallbackPageTitle(relPath: string): string {
