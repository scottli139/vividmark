...
- **文件关联（Open With）**: `tauri.conf.json` `bundle.fileAssociations`（仅打包安装后生效，LaunchServices 注册）；macOS `RunEvent::Opened` → `route_open_paths` 窗口路由 → `src/lib/openWith.ts`（启动积压由 `take_startup_open_files` 按 label 补取）；Windows/Linux 由 setup 的 `collect_argv_files()` 解析启动 argv 走同一路由（冷启动入 main 启动队列）；单实例未接（运行中再双击会起第二进程，后续项）
