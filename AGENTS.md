...
- **Linux 无边框自绘标题栏**: Linux 下主窗口（lib.rs setup）与新文档窗口（window_router）均 `decorations(false)`——SSD 标题栏+GTK 菜单栏+工具栏三条过厚，且 X11/SSD 下主题分割线不生效；顶部减为菜单栏+工具栏两条。**GTK 菜单栏必须保留**（快捷键全靠 accelerator）。前端 `isLinuxDesktop()` 门控：Toolbar 自绘居中标题 + `WindowControls.tsx`（最小化/最大化切换/关闭，关闭走 `win.close()` 与脏确认同路径）；拖拽/双击最大化靠 tauri drag.js 零新增代码；边缘缩放靠 `WindowResizeHandles.tsx`（四边 4px/四角 12px 热区 → `startResizeDragging`，最大化时隐藏；`ResizeDirection` 类型 api 2.10 未导出需本地复刻）。capabilities 需 `allow-minimize`/`allow-close`/`allow-toggle-maximize`/`allow-internal-toggle-maximize`/`allow-is-maximized`/`allow-start-resize-dragging`/`allow-set-fullscreen`/`allow-is-fullscreen`（后两项供 `view:fullscreen` 自定义菜单项）
