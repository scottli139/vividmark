...
- **自动保存** - 无操作 2 秒后自动保存
- **文件变更监控** - 外部修改在缓冲区干净时自动重载；有未保存更改时由你选择重新加载或保留（抉择期间自动保存暂停）；外部删除保留内容、下次保存重建文件
- **最近文件** - 快速访问最近打开的文件
...
- **感知 VuePress 项目（尽力而为）** - 目录中存在 `.vuepress/` 时：正则提取 config 的 `title` 作站点名，`.vuepress/public/*` 原样镜像到站点根；sidebar 是可执行 JS 不做解析，导航退回目录结构推导
...
- **脚注** - GFM 风格 `[^id]` 引用，定义集中渲染到文末并带双向回链；编号按引用首现顺序，WYSIWYG 与预览一致（同一定义的多次引用共享编号）：
...
- **排版增强** - `==高亮==`、`^上标^`、`~下标~` 与 emoji 短码（`:smile:`）；高亮/上标/下标在 WYSIWYG 中是真实行内格式（键入闭合分隔符自动转换，保存后源码不变），emoji 短码在 WYSIWYG 保持字面文本、预览与导出中渲染（单 `~` 是下标，删除线需用 `~~`）：

  ```markdown
  ==高亮==，E = mc^2^，H~2~O，:rocket:
  ```

- **PlantUML 图表** - 内置本地引擎（@plantuml/core）直接在文档中渲染 UML 图表 —— 完全离线、暗色自适应，PDF/站点导出内联 SVG（仅本地渲染失败时回退在线服务）
- **Mermaid 图表** - 内置 mermaid.js 渲染流程图、时序图、甘特图等 —— 首个图表按需懒加载、完全离线、暗色自适应，PDF/站点导出内联 SVG（语法错误显示内联错误态，无在线回退）
  ```markdown
  @startuml
  Alice -> Bob: 你好
  Bob --> Alice: 嗨！
...
- **YAML Frontmatter** - 文档顶部的元数据块：预览 / PDF / 站点导出中自动剥离，WYSIWYG 显示为只读卡片（编辑请用源码模式），站点导出时 `title` 作为页面标题
...
- **扩展语法**: markdown-it-container, @plantuml/core（离线 UML 引擎）, mermaid（离线图表）, KaTeX
- **语法高亮**: highlight.js
...
- [x] 第五阶段: 文件管理（文件树 ✅、多窗口（Typora 式 SDI）✅、文件变更监控 ✅）
