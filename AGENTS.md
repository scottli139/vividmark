...
**完整快捷键表见 `docs/implementation-notes.md`「键盘快捷键一览」**。核心约定：macOS/Linux 桌面端带 accelerator 的键被 OS 拦截（webview 收不到 keydown），快捷键由原生菜单事件驱动；**Windows 桌面端（无原生菜单）与浏览器 dev/E2E 由 `useKeyboardShortcuts.ts` 全量接管**（键位表与 menu.rs 对齐，编辑器自家 keymap 已处理的键经 `e.defaultPrevented` 守卫跳过）。
...
- **多窗口（Typora 式 SDI）**: 每文档独立窗口（独立 webview/JS 上下文）。核心 `src-tauri/src/window_router.rs`：注册表 `WINDOW_STATES`（前端 `report_window_state` 上报）+ `LAST_FOCUSED` + 启动待打开队列；打开路由 = 已打开→聚焦 / 干净空窗口→复用 / 否则新建（pending 去重）。**Windows 建窗命令必须 `#[tauri::command(async)]`**（同步命令在主线程执行，主线程 build() 在事件循环运转后自死锁——0.9.0「新建窗口假死」hotfix；macOS/Linux 保持主线程建窗，worker 上经 `run_on_main_thread` 派发，setup 记录主线程 id 分流）。**跨窗口偏好同步走 `prefs-sync` tauri 事件广播——不能用 localStorage storage 事件**（macOS WKWebView 多窗口 localStorage 各自独立）；菜单/Dock 事件经 `emit_to_focused` 定向；capabilities 需 `windows: ["*"]`；PDF 隐藏窗口 label 排除在路由外
- **原生菜单事件流（仅 macOS/Linux；Windows 无原生菜单）**: `menu.rs` 构建 → `on_menu_event` 经 `emit_to_focused` emit `native-menu-event` → `nativeMenu.ts handleMenuAction` 分发（macOS/Linux 桌面端快捷键全走此路，见上「Keyboard Shortcuts」）。id 约定与右键菜单同源（`format:<FormatType>` / `insert:*`）；查子菜单项必须走 lib.rs `find_menu_item` 递归（`Menu::get` 只查顶层）；CheckMenuItem 点击会原生自动翻转勾选，最终态以同步为准；**菜单重建后 check/enabled 回到构建默认值，必须重同步一轮**；**聚焦时必须补 rebuildMenu**（启动时 isFocused 可能为 false 导致初始重建跳过，菜单标签停留在构建默认语言）；rebuildMenu 失败要重置去重 key 允许重试。Edit 的 Undo/Redo 用自定义项（系统级 undo 会绕过编辑器 history）。**muda GTK 预定义项白名单**：Linux 后端仅放行 `Separator`/剪贴板四件/`About`，`Minimize`/`Maximize`/`Quit`/`CloseWindow`/`Fullscreen` 静默丢弃——Linux 下这些一律用自定义项（`window:minimize`/`window:maximize`/`view:fullscreen`/`file:exit`→`quit_app` 命令逐窗口 close 走脏确认）。**菜单栏文字右偏**：muda 给每项打包 16px 空 GtkImage 占位把文字右推 22px，`strip_menubar_icon_placeholders()`（lib.rs）摘除空图，setup / rebuild_menu / 建新窗口三处调用
...
- **文件关联（Open With）**: `tauri.conf.json` `bundle.fileAssociations`（仅打包安装后生效，LaunchServices 注册）；macOS `RunEvent::Opened` → `route_open_paths` 窗口路由 → `src/lib/openWith.ts`（启动积压由 `take_startup_open_files` 补取——**必须传本窗口 label**，None 会清空全部窗口队列）；**冷启动 Opened 可早于 config 建窗/前端上报到达——`RunEvent::Ready` 前一律入队 main 启动队列而非新建窗口**（APP_READY 标志，否则必现多余的欢迎页窗口）；Windows/Linux 由 setup 的 `collect_argv_files()` 解析启动 argv 走同一路由（冷启动入 main 启动队列）；单实例未接（运行中再双击会起第二进程，后续项）
...
- **菜单原语**: 下拉/右键/菜单栏统一用 `src/components/Menu/`（Dropdown / ContextMenu / MenuPanel / MenuBar），禁止再复制 outside-click 模式；ContextMenu 的 `onClose` 必须 useCallback 稳定化；MenuPanel 支持一层子菜单（`MenuSubmenuItem.children`，右缘翻左、底部上收）；**原语均无键盘导航**（仅 Escape），Alt 助记符/方向键导航是统一后续项
...
- **Linux/Windows 无边框自绘标题栏**: 去装饰方式分平台——Linux 主窗口（lib.rs setup）与新文档窗口（window_router）建窗时 `decorations(false)`；**Windows 一律建窗成功后再 `set_decorations(false)`**（与验证过的主窗口形态保持一致）——系统标题栏+菜单栏+工具栏三条堆叠过厚。前端 Toolbar 兼作标题栏：自绘文件名 + `WindowControls.tsx`（最小化/最大化切换/关闭，关闭走 `win.close()` 与脏确认同路径）；拖拽/双击最大化靠 tauri drag.js 零新增代码；边缘缩放靠 `WindowResizeHandles.tsx`（四边 4px/四角 12px 热区 → `startResizeDragging`，最大化时隐藏；`ResizeDirection` 类型 api 2.10 未导出需本地复刻）。门控 `isLinuxDesktop()`/`isWindowsDesktop()`（platform.ts）。capabilities 需 `allow-minimize`/`allow-close`/`allow-toggle-maximize`/`allow-internal-toggle-maximize`/`allow-is-maximized`/`allow-start-resize-dragging`/`allow-set-fullscreen`/`allow-is-fullscreen`（后两项供 `view:fullscreen` 菜单项）。**平台差异：Linux 保留 GTK 菜单栏**（快捷键全靠 accelerator；GTK 后端另有空图标占位/deepin padding 两处修复，见 notes）；**Windows 连菜单栏一起去掉**（setup 不 set_menu、rebuild_menu no-op），菜单与快捷键全部前端化，见下条
- **Windows 自绘菜单栏（MenuBar）**: `src/components/Menu/MenuBar.tsx` 嵌在 Toolbar 左组（VS Code 风格单行：菜单+标题+MoreMenu+窗口控制），**浏览器 dev/E2E 同样渲染**（`!isMacOSDesktop() && !isLinuxDesktop()`，E2E 直接可测 `e2e/menubar.spec.ts`）。菜单项构建是纯函数 `src/lib/menuBar.ts`（结构对齐 menu.rs Windows 布局，id 与原生菜单同源，checked/disabled 响应式直读 store——**不需要** set_menu_item_checked 那套命令式同步）；点击仍走 `handleMenuAction`。原生预定义项的前端平替：Cut/Copy/Paste/SelectAll → `editor-cut/copy/paste/select-all` 事件（三编辑器各自实现，复用右键菜单逻辑）；About → `AboutDialog.tsx`（store `isAboutOpen`）；Minimize/Maximize/Fullscreen/Exit → 复用 Linux 的 `window:*`/`view:fullscreen`/`file:exit` id。`initNativeMenu` 在 Windows 整体跳过（仅保留 handleMenuAction 导出）
